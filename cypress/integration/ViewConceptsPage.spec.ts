...
describe("View Concepts Page", () => {
  ...
  //   it("should import concepts from source", () => {
  //     cy.visit(`/users/${currentUser()}/collections/${dictionaryId}`);

  //     cy.find('[data-testid="addConceptsIcon"]').click();
  //     cy.find(`[text=${TEXT.IMPORT_EXISTING_CONCEPT}`).click();
  //     cy.find(`[text=${TEXT.PICK_CONCEPTS}]`).click();

  //     cy.find(`[title="More actions"]`)
  //       .first()
  //       .click();
      
  //     cy.find(`[text=${TEXT.ADD_TO_DICTIONARY}]`)
  //       .click();
  //   });
  // });
